        ...
        custom: {
            options: async (value, { req }) => {
                const user = await databaseService.users.findOne({
                    where:{
                        [Op.and]: [{email: value}, {password: hashPassword(req.body.password)}]
                    }
                });
                if (user === null) {
                ...
                if (user.verified === false) {
                    throw new Error(USERS_MESSAGES.EMAIL_NOT_VERIFIED);
                }
                req.user = user;
                return true;
            }
            ...
            options: async (value, { req }) => {
                ...
                try {
                    const [decoded_refresh_token, refresh_token] = await Promise.all([
                        ...
                        databaseService.refreshTokens.findOne({where: { token: value }})
                    ]);
